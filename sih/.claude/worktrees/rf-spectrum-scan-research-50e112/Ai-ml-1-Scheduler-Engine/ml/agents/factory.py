...
# Policies with no ml/configs/<name>.yaml of their own: every constant they use is derived from a
# mission requirement rather than tuned, so there is nothing for a hyperparameter file to hold.
DERIVED_POLICIES = ("index", "ctmc")
...
def build_agent(
    ...
    **kwargs,
    ...
    if policy in DERIVED_POLICIES:
        cfg = dict(hyperparams or {})
        cfg.update(kwargs)
        cls = IndexAgent if policy == "index" else CTMCFloorAgent
        return cls(num_bands, rng=rng, **cfg)
