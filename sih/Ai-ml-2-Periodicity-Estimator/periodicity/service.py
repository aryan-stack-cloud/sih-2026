...
class PeriodicityService:
    ...
    def observe_miss(self, simulation_id: str, band_id: int, timestamp: float) -> None:
        """Record a scan that found nothing, and invalidate that band's cached fit.
    ...
    def estimate(self, simulation_id: str, band_id: int) -> PeriodEstimate:
        ...
        misses = self.buffers.snapshot_misses(simulation_id, band_id)
        if misses:
            observations = self.buffers.snapshot_observations(simulation_id, band_id)
