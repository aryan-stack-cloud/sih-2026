...
class UpdateRequest(BaseModel):
    """``POST /internal/periodicity/update`` -- one scan outcome for a band.

    ``detected`` defaults to true, so every caller written against the original hits-only
    contract keeps working unchanged. Solution spec Section 3.2: reporting the scans that found
    *nothing* too (``detected=false``) is what lets the estimator use hits and misses together
    -- see estimate_period_with_misses. ``detection_timestamp`` is the scan time either way.
    """
