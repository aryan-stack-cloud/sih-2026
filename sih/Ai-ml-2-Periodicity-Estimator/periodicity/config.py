...
class EstimatorConfig:
    ...
    # -- hits-and-misses likelihood (solution spec Section 3.2) -----------------------------
    detect_p_d: float = 0.933
    """Receiver detection probability, for the log-likelihood in estimate_period_with_misses.
    Default derived from the reference scenario's Gaussian-threshold detector (threshold=1.5,
    snr_mean=3.0, noise_sigma=1.0): P_d = norm.sf((threshold - snr_mean) / noise_sigma). Matches
    how Ai-ml-1's IndexAgent.from_scenario derives the same quantity from the same receiver
    model, so the two services agree on what the detector actually does."""

    detect_p_fa: float = 0.067
    """Receiver false-alarm probability: P_fa = norm.sf(threshold / noise_sigma), same reference
    receiver as detect_p_d."""

    duty_candidates: tuple[float, ...] = (0.05, 0.1, 0.2, 0.3, 0.5)
    """Candidate illuminated-fraction values the hits-and-misses grid search tries per period."""

    phase_grid_points: int = 24
    """Phase candidates per period in the hits-and-misses grid search's wide sweep (refined
    locally afterwards at 4x this resolution around the winner). Runtime scales linearly with
