...
def test_update_accepts_a_miss_and_it_sharpens_the_estimate(client):
    """detected=false is the Section 3.2 addition: a scan that found nothing is still posted to
    /internal/periodicity/update, and feeding enough of them recovers a period from far fewer
    hits than the hits-only path needs."""
    sim = "sim_periodic"
    band = 9
    for t in range(60):
        detected = (t % 20) in (0, 1)
        r = client.post(
            "/internal/periodicity/update",
            json={
                "simulation_id": sim,
                "band_id": band,
                "detection_timestamp": float(t),
                "detected": detected,
            },
        )
        assert r.status_code == 200
    ...
    state = client.get(
        "/internal/periodicity/state", params={"simulation_id": sim, "band_id": band}
    ).json()["data"]
    assert state["estimate"]["period"] == pytest.approx(20, abs=1.0)
    assert state["estimate"]["samples"] < 8  # well under the hits-only min_samples gate


def test_update_without_detected_still_behaves_as_a_pure_detection_event():
    """Backward compatibility: a caller that never sends `detected` gets exactly the old
    hits-only behaviour, unchanged."""
    from periodicity.service import PeriodicityService
    from periodicity.contract import UpdateRequest

    svc = PeriodicityService()
    body = UpdateRequest(simulation_id="sim_compat", band_id=1, detection_timestamp=5.0)
    assert body.detected is True
    svc.update(body.simulation_id, body.band_id, body.detection_timestamp)
    assert svc.buffers.snapshot("sim_compat", 1) == [5.0]
    assert svc.buffers.snapshot_misses("sim_compat", 1) == []
    svc.reset("sim_compat")
