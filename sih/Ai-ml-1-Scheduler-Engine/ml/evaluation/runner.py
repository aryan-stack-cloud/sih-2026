...
One place owns the (reset -> select_action -> step -> observe -> learn) loop so that a policy is scored the
...
def run_episode(
    ...
    while True:
        ...
        # Raw outcome first: a belief-tracking policy updates on hits and misses, and must see
        # them whether or not this is a learning pass. No-op for every value-learning agent.
        agent.observe(info)
        if learn:
