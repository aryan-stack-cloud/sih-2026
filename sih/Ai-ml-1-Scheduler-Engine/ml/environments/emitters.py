...
def _validate_bands(bands, num_bands: int) -> tuple[int, ...]:
    out = tuple(int(b) for b in bands)
    if not out:
        raise ValueError("a pinned band list must name at least one band")
    for band in out:
        if not 0 <= band < num_bands:
            raise ValueError(
                f"pinned band {band} is outside the {num_bands}-band spectrum"
            )
    return out
...
def _assign_bands(
    ...
    """Which bands an emitter occupies, pinned by the scenario or drawn at random.

    Pinning matters for scenarios that have to be *arithmetically* reproducible rather than
    merely seeded. The synchronisation trap is the example: a stride-1 sweep over 8 bands is
    locked out of an 8-step emitter at every band but one, so leaving the band to the dice makes
    the scenario demonstrate the trap only seven times in eight.
    """
    if behavior == "agile":
        if params.get("bands") is not None:
            return _validate_bands(params["bands"], num_bands)
    ...
    if params.get("band") is not None:
        return _validate_bands([params["band"]], num_bands)
