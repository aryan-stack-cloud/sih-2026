...
POLICY_TYPES = ("baseline", "ctmc", "index", "bandit", "q_learning", "dqn", "ppo")
...
class Agent(abc.ABC):
    ...
    def observe(self, info: dict) -> None:
        """Consume one step's raw outcome: which bands were scanned, which of them detected.

        ``learn()`` carries a scalar reward, which suits a value-learning agent and does not suit
        a belief-tracking one -- a Bayesian update needs the hits and the misses themselves, not
        their weighted sum. The environment already puts both in its ``info`` dict, so this hook
        just hands them over. No-op by default, so no existing policy changes behaviour.
        """
        return None
