...
class IndexAgent(Agent):
    ...
    # -- persistence ---------------------------------------------------------------------------
    #
    # There is nothing learned to snapshot: the belief and kernel state are per-simulation online
    # state, rebuilt fresh each run (see the class docstring). What the model registry needs to
    # reproduce is the configuration a run was built with -- the receiver-derived parameters plus
    # the six tuned index scalars, exactly what scripts/tune_index.py's ``--write`` output already
    # contains. Saving/loading that config is what makes ``index`` a first-class registry entry,
    # loadable through the same ``ModelRegistry.load_agent`` path as every other policy.
    ...
    def save(self, path: str | Path) -> None:
        ...
        config = {
            ...
            "mode": self.mode,
            "slot_s": self.slot_s,
            "retune_s": self.retune_s,
            "deadline_s": float(self.deadlines.deadlines_s[0]),
            "detect_p_fa": self.detect_p_fa,
            "detect_p_d": self.detect_p_d,
            "rho_min": self.rho_min,
            "rho_max": self.rho_max,
            "floor_mean_dwell_slots": self.floor.mean_dwell_slots,
            "floor_confidence_threshold": self.floor_confidence_threshold,
            "weights": {
                "w_value": self.weights.w_value,
                "w_info": self.weights.w_info,
                "w_novelty": self.weights.w_novelty,
                "w_deadline": self.weights.w_deadline,
            },
        }
        path.write_text(json.dumps(config, indent=2), encoding="utf-8")

    @classmethod
    def load(cls, path: str | Path, **kwargs) -> "IndexAgent":
        config = json.loads(Path(path).read_text(encoding="utf-8"))
        weights = IndexWeights(**config.pop("weights"))
