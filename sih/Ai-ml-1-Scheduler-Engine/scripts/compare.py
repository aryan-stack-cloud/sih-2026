...
def run_policy_for(scenario: dict, policy: str, episodes: int | None, env_factory=None) -> dict:
    ...
    if policy in ("baseline", "random"):
        ...
        return summary
    if policy in ("index", "ctmc"):
        # Derived policies have nothing to train. They do need the scenario receiver block, so
        # that the belief update runs on the Pd and Pfa this receiver actually delivers rather
        # than on library defaults.
        from ml.agents.ctmc_floor import CTMCFloorAgent
        from ml.agents.index_agent import IndexAgent

        agent = (
            IndexAgent.from_scenario(scenario)
            if policy == "index"
            else CTMCFloorAgent(scenario["bands"])
        )
        summary = evaluate(agent, scenario, seeds=seeds)
        summary["policy"] = policy
        return summary
