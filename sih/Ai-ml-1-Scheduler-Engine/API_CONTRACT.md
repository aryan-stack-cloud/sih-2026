...
The first eight per-band fields are the original ML-001 set and are **required**. The five below
the blank line were added for the `index` policy (SCT) and are **optional with defaults**, so a
Backend that has not adopted them yet keeps working unchanged:

| Field | Type | Default | Meaning |
|---|---|---|---|
| `revisit_deadline` | int, steps | `0` = no deadline | Longest revisit interval that still meets this band's worst-case intercept requirement. Derived from intercept-time theory, not tuned. |
| `staleness_ratio` | float, 0-1+ | `0.0` | `time_since_last_scan / revisit_deadline`. At or above 1 the band is overdue and the scheduler must include it. |
| `threat_score` | float, 0-1 | `0.5` | Learned emitter-character threat weight. Distinct from `band_priority_weight`, which stays the operator-set value. |
| `occupancy_belief` | float, 0-1 | `0.5` | Posterior probability the band is active now, from the two-state occupancy chain. |
| `sync_risk` | float, 0-1 | `0.0` | Posterior weight on period hypotheses this revisit interval can never intercept. Non-zero means the current schedule is locked out. |
...
### `detected: false` — reporting scans that found nothing

Additive, backward-compatible: `detected` defaults to `true`, so a caller that never sends it
gets exactly the original hits-only behaviour. Sending `detected: false` on every scan (not just
confirmed detections) is what lets the estimator distinguish "this band is quiet" from "this band
was never looked at" — the gap RUNNING.md documents as the reason periodicity prediction did not
yet measurably improve detection latency (PRD DoD item 8). The Backend should call this on
**every** scan outcome for the scheduled band(s), hit or miss, not only on confirmed detections.
...
policy_type:     baseline | ctmc | index | bandit | q_learning | dqn | ppo
...
New in this revision: `ctmc` (the randomised floor, also baseline arm B4) and
`index` (the SCT scheduler). Both are additive; no existing value changes meaning.
```
